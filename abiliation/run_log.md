...
| abl_saf | run/abl_saf | 2503504 | 42 | 100 | | | | row 3: SAF-SIoU alone (args.yaml box_loss_type: saf_siou), attn none, stock BCE — only delta vs baseline. First row with a committed config diff (not env-var). SAF loss de-risked locally. Awaiting run |

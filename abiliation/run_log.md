...
| abl_no_cbam | run/abl_no_cbam | | 42 | 100 | | | | row 5 (leave-one-out, Full − CBAM): POLLENBEES_ATTN=botnet (BoT on, CBAM off) + SAF-SIoU + VFL-w. args.yaml identical to abl_full; only delta is dropping CBAM. Same backbone as abl_bot (~22.6M params), but with full-stack losses. Isolates CBAM's marginal gain on top of BoT+SAF+VFL — if AP ≈ abl_full within noise, CBAM is redundant. Awaiting run |

...
| abl_vflw | run/abl_vflw | PENDING | 42 | 100 | | | | row 4: weighted VFL alone (args.yaml cls_loss_type: vfl + cls_pos_weight [1,2]), attn none, box back to CIoU — only delta vs baseline (§2: VFL+weighting as one bundle). VFL-w loss de-risked locally. Awaiting run |

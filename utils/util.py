...
class ComputeLoss:
    def __init__(self, model, params):
        ...
        if self.cls_loss_type == 'bce':
            ...
            raise ValueError(f"Unsupported cls_loss_type: {self.cls_loss_type}. Use 'bce' or 'vfl'.")
    ...
    def __call__(self, outputs, targets):
        ...
        loss_cls_raw = self.cls_loss(pred_scores, target_scores)
        if self.cls_loss_type == 'vfl' and self.cls_pos_weight is not None:
            pos_weight = self.cls_pos_weight.to(dtype=data_type)
            loss_cls_raw = loss_cls_raw * torch.where(target_scores > 0, pos_weight, torch.ones_like(target_scores))
        loss_cls = loss_cls_raw.sum() / target_scores_sum
